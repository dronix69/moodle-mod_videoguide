...
/**
 * Upgrade the videoguide database.
 *
...
function xmldb_videoguide_upgrade($oldversion) {
    ...
    if ($oldversion < 2026090600) {
        // Add the displaymethod field: how videos open (newtab, modal, popup).
        $table = new xmldb_table('videoguide');
        $field = new xmldb_field('displaymethod', XMLDB_TYPE_CHAR, '20', null, XMLDB_NOTNULL, null, 'newtab', 'introformat');

        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_mod_savepoint(true, 2026090600, 'videoguide');
    }
